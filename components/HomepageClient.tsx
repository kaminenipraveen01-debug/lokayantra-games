  ...
  return (
    <main className="w-full min-h-screen text-black font-sans pb-12 relative overflow-hidden select-none bg-[#cfcfcf]">
      ...
      <div className={`w-full max-w-[1400px] mx-auto px-3 sm:px-4 ${recentGames.length > 0 ? "mt-6 sm:mt-8" : "mt-[105px] sm:mt-[115px]"} relative z-10`}>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-1.5 sm:gap-2 grid-flow-row-dense">
  {games.map((game, index) => {
    ...
    return (
      <Link
        ...
        key={game.id}
        className={`group relative overflow-hidden rounded-[16px] sm:rounded-[20px] bg-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-150 ${sizeClass}`}
      >
        {game.thumbnail ? (
          <img
            src={game.thumbnail}
