...
export default function SearchResultsClient({ initialGames, newestGames, categories }: Props) {
  ...
  return (
    <div className="w-full">
      {/* SEARCH + FILTER BAR */}
      <div className="bg-white/5 backdrop-blur-2xl rounded-[24px] sm:rounded-[32px] border border-white/10 p-5 sm:p-7 shadow-sm mb-6 flex flex-col sm:flex-row gap-3">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full h-12 pl-11 pr-4 rounded-2xl bg-white border border-black/10 focus:border-black outline-none font-bold text-sm text-black placeholder-black/40 transition-all"
          />
        </div>
        ...
        <select
          ...
          className="h-12 px-4 rounded-2xl bg-white border border-black/10 focus:border-black outline-none font-black text-[11px] uppercase tracking-wide text-black cursor-pointer"
        >
      ...
      {/* RESULT COUNT */}
      <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-4 px-1">
      ...
      {results.length > 0 ? (
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-1.5 sm:gap-2">
          {results.map((game) => (
            <Link
              ...
              key={game.id}
              className="group relative aspect-square overflow-hidden rounded-[20px] sm:rounded-[24px] border border-white/10 hover:border-white/30 bg-white/5 hover:bg-white/10 shadow-[0_4px_10px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_24px_rgba(0,0,0,0.18)] hover:-translate-y-1.5 transition-all duration-200"
            >
              {game.thumbnail ? (
                <img
                  src={game.thumbnail}
                ...
                <div className="absolute inset-0 flex items-center justify-center p-2 text-center text-[10px] font-black uppercase tracking-wider text-white/60">
        ...
        <div className="text-center font-bold py-20 bg-white/5 rounded-[24px] border border-white/10">
          <p className="text-sm font-black uppercase tracking-wide text-white/50 mb-1">No games match your search</p>
          <p className="text-xs text-white/40 font-semibold">Try a different keyword or clear the category filter.</p>
        </div>
      )}
    </div>
  );
